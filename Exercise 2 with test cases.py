...
# The algorithm runs in linear time - O(n+n+constants) which we can simplify to O(n).<br><br>
# #### Test case - we calculate the running time empirically
...
# We put running time of each step in the comments. We did not take into account the constant time because it is not significant if we already have O(n). The running time of the whole algorithm is O(m+m+m+n*((m+n)+m+m+(m+m)+m+m) which we can simplify to O(n^2+m*n).<br><br>
# #### Test case - we calculate the running time empirically
